...
const dataArray = [
  {label: 'Followers', value: followers, icons:User},
  {label: 'Following', value: following, icons:UserCheck},
  {label: 'Pending', value: pendingConnections, icons:UserRoundPen},
  {label: 'Connectios', value: connections, icons:UserPlus},
]

const [currentTab, setCurrentTab] = useState('Followers', 'Following','Pending', 'Connections')
          ...
          <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
            {dataArray.map((tab)=>(
              <button onClick={()=> setCurrentTab(tab.label)} key={tab.label}
               ...
               <span className='ml-l'>{tab.label}</span>
               {tab.count !== undefined && (
          ...
          <div className='flex flex-wrap gap-6 mt-6'>
             {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
              <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
                         <img src={user.profile_picture} className='rounded-full w-12 h-12 shadow mx-auto' alt="" />
                   <div className='flex-1'>
                     ...
                     <div className='flex max-sm:flex-col gap-2 mt-4'>
                     {
                      ...
                      className='w-full p-2 text-sm rounded bg-gradient-to-r from-indigo-500
                       to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 
