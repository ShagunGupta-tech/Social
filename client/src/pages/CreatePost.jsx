...
const CreatePost = () => {
  ...
  return (
   ...
   <div className='max-w-xl bg-white p-4 sm:pb-3 rounded-xl shadow-md space-y-4'>
    ...
    {/* Text Area  */}
    <textarea className='w-full resize-none max-h-20 text-sm outline-none placeholder-gray-400 '
    placeholder="What's happening" onChange={(e)=>setContent(e.target.value)} value={content}/>
   
   {/* Images  */}
   {
    images.length > 0 && <div className='flex fkex-wrap gap-2 mt-4'>
      {
        images.map((image, i)=>(
          <div key={i} className='relative group'>
           <img src={URL.createObjectURL(image)} alt="" className='h-20 rounded-md'/>
           <div onClick={()=> setImages(images.filter((_, index)=> index !== i))} 
           className='absolute hidden group-hover:flex justify-center items-center top-0 right-0 bott-0
   ...
   <div className='flex items-center justify-between pt-3 border-t border-gray-300'>
        <label htmlFor="images" className='flex items-center gap-2 text-sm text-gray-700 transition cursor-pointer'>
        ...
        })} className='text-sm bg-gradient-to-r from-indigo-500 to-purple-600 hover:indigo-600 hover:to-purple-700 
